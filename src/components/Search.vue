<template>
    <div id="search-bar">
        <span id="search-close-button" class="material-icons" @click="closeSearchMenu()">close</span>
        <form id="search-form" @submit.prevent="searchCity()">
            <label for="location" id="location-label"><span class="material-icons">search</span>Search location</label>
            <input type="text" name="location" id="search-location" @focus="hideLabel()" @blur="showLabel()"/>
            <input type="submit" name="search" id="search-button" value="Search"/>
        </form>

        <div id="search-results">
            <span v-for="result in results" :key="result.id">
                <div class="result-button" @click="setCity(result.name, result.lat, result.lon), closeSearchMenu()">
...
export default {
    ...
    setup(){
        ...
        return {results, searchCity, hideLabel, showLabel, setCity, closeSearchMenu};
    }
}
</script>